...
double getSamplingRate(std::string cmdb, std::string env){
    ...
    if((cur_timestamp - lastUpdatedTime) > 180000 ){
      ...
      std::string cur_url = env == "prod"?"http://internal-ms-service-discovery-887102973.ap-northeast-2.elb.amazonaws.com/":"http://internal-ms-consul-alb-416054892.ap-northeast-2.elb.amazonaws.com:8500/ui/:8500";
      ppconsul::Consul consul(cur_url,kw::token=cur_token);
      Kv kv(consul,kw::token=cur_token);
      
      return stod(kv.get("hot_config/coutrace/nginx/" +  cmdb, "0.01", kw::token=cur_token));
    }
    return -1.0;
}
